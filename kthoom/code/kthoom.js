...
class KthoomApp {
  ...
  parseParams_() {
    ...
    if (bookUri) {
      ...
      if (bookUri.indexOf('ipfs://') === 0) {
      ...
      } else if (bookUri.indexOf('dp') === 0) {
        const xhr = new XMLHttpRequest();
        xhr.open('GET', '/kthoom/ajax/zip_list', true);
        ...
        xhr.addEventListener('load', function(e){
          ...
          for (var i in tmp) {
            const xhr = new XMLHttpRequest();
            xhr.open('GET', tmp[i], true);
            var name_tmp = tmp[i].split('/');
            var name = name_tmp[name_tmp.length-1];
            theApp.loadSingleBookFromXHR(name, xhr, -1);
            /*
           ...
           /*
            bookPromiseChain = bookPromiseChain.then(() => {
              return Book.fromXhr(name, xhr, -1).then(book => {
            ...
            bookPromiseChain.then(() => {
              if (books.length > 0) {
                theApp.readingStack_.addBooks(books, true);
              }
              if (foundError) {
                console.log('Could not open all books. See the console for more info.');
              }
            });
            */
          }
          //if (theApp.readingStack_.getNumberOfBooks()>0)
          theApp.readingStack_.changeToBook(0);
        })
        xhr.send(null);
      } else {
        ...
        //console.log(bookUri);
        var tmp = bookUri.split('|')
        for (var i in tmp) {
          const xhr = new XMLHttpRequest();
          xhr.open('GET', tmp[i], true);
          var name_tmp = tmp[i].split('/');
          var name = name_tmp[name_tmp.length-1];
          this.loadSingleBookFromXHR(name, xhr, -1);
        }
      }
    } else {
